...
DEFAULT_CONFIG = {
    ...
    # --- complex test cases (the falsifiability gate) ---
    # A promise's proof is only as strong as the check behind it, and a
    # one-line `--accept` written by the same agent that wants it to pass is
    # not a gate — it is decoration. A COMPLEX TEST CASE SUITE replaces it:
    # several cases, each in a named dimension, each PROVEN FALSIFIABLE (seen
    # RED before the work, so it is known to be capable of failing).
    #   require_cases      — no promise may bank a win without a green,
    #                        falsifiable suite (recommended; default off only
    #                        so existing projects keep working)
    #   min_cases          — the size floor: fewer is not a suite
    #   require_dimensions — dimensions the suite MUST cover. A suite that is
    #                        all happy-path proves the happy path and nothing
    #                        else; `negative` (the system must correctly REFUSE
    #                        bad input) is the cheapest defence against a check
    #                        that passes for the wrong reason.
    "require_cases": False,
    "min_cases": 3,
    "require_dimensions": ["happy", "negative"],
}

IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp", ".tiff",
              ".heic", ".pdf"}

# The dimensions a test case can occupy. A suite that does not span several of
# these is not testing the promise — it is testing the one path the author
# happened to have in mind.
DIMENSIONS = {
    "happy": "the promise honored on the intended path",
    "negative": "the system correctly REFUSES bad input / denies what it "
                "must deny (a check that never says no cannot say yes)",
    "edge": "a boundary: empty, zero, max, unicode, concurrent, missing",
    "regression": "the specific bug this promise fixes, asserted to stay dead",
    "integration": "end-to-end through the REAL surface — no mock, no stub, "
                   "the thing a stakeholder would actually touch",
}
...
class Project:
    ...
    def __init__(self, sc: Path):
        ...
        self._case_events = read_jsonl(sc / "cases.jsonl")
    ...
    # -- complex test cases ---------------------------------------------------
    def case_suite(self, pid: str) -> dict[str, dict]:
        """The CURRENT suite for a promise: {case_name: {dim, run, why, ...}}.
        Later `case` events supersede earlier ones with the same name (that is
        what an amendment IS), so file order — which is ts order — decides."""
        out: dict[str, dict] = {}
        for e in self._case_events:
            if e.get("type") == "case" and e.get("id") == pid:
                out[e["case"]] = {k: e[k] for k in
                                  ("dim", "run", "why", "amended", "reason",
                                   "by") if k in e}
        return out

    def case_history(self, pid: str) -> list[dict]:
        """Every recorded run of this promise's suite, oldest first. This is
        the falsifiability record — the only thing that can prove a case was
        ever ABLE to fail."""
        return [e for e in self._case_events
                if e.get("type") == "caserun" and e.get("id") == pid]

    def falsifiable_cases(self, pid: str) -> set[str]:
        """Cases OBSERVED RED at least once, against the exact command they
        carry today. This is the heart of the feature.

        A test that has never failed has never been tested. It may be asserting
        something true by construction, it may be grepping a string that is
        always there, it may be a negative case that passes because the feature
        does not exist yet. The only evidence that a check can detect the
        absence of the thing is having WATCHED it detect the absence — i.e. a
        RED observation from before the work landed.

        Keying on the command text (not just the case name) is what closes the
        tampering hole: weaken a case to make it pass and it becomes, to this
        function, a brand-new case with no red history — VACUOUS, and it cannot
        bank a win. The only way back to green is to make it red first, which
        means reverting the code. Honest either way."""
        return {e["case"] for run in self.case_history(pid)
                for e in run.get("results", [])
                if e.get("status") == "red"
                and e.get("run") == self.case_suite(pid)
                .get(e["case"], {}).get("run")}

    def case_status(self, pid: str) -> dict:
        """The full verdict on a promise's suite. `ok` is the bank-a-win gate.

        Four ways a suite fails, and they are different failures:
          NO_SUITE  — nothing to check
          SHALLOW   — too few cases, or missing a required dimension: the suite
                      proves one path, not the promise
          VACUOUS   — a case has never been seen RED: it is not known to be
                      capable of failing, so its green means nothing
          RED       — a case is failing right now: the promise is not honored
        """
        suite = self.case_suite(pid)
        if not suite:
            return {"id": pid, "verdict": "NO_SUITE", "ok": False,
                    "cases": {}, "green": 0, "total": 0, "vacuous": [],
                    "red": [], "missing_dims": [], "amended": [],
                    "reason": "no test cases authored for this promise"}
        hist = self.case_history(pid)
        last = {e["case"]: e["status"]
                for e in (hist[-1]["results"] if hist else [])}
        falsifiable = self.falsifiable_cases(pid)
        vacuous = sorted(set(suite) - falsifiable)
        red = sorted(c for c in suite if last.get(c) != "green")
        dims = {c["dim"] for c in suite.values()}
        need = [d for d in (self.config.get("require_dimensions") or [])
                if d not in dims]
        min_n = int(self.config.get("min_cases", 0) or 0)
        amended = sorted(c for c, v in suite.items() if v.get("amended"))
        green = sum(1 for c in suite if last.get(c) == "green")
        out = {"id": pid, "cases": suite, "green": green, "total": len(suite),
               "vacuous": vacuous, "red": red, "missing_dims": need,
               "amended": amended, "dims": sorted(dims)}
        if len(suite) < min_n or need:
            short = []
            if len(suite) < min_n:
                short.append(f"{len(suite)} case(s), floor is {min_n}")
            if need:
                short.append("no " + "/".join(need) + " case")
            return {**out, "verdict": "SHALLOW", "ok": False,
                    "reason": "; ".join(short) + " — a suite that does not "
                              "span dimensions proves one path, not the promise"}
        if red:
            return {**out, "verdict": "RED", "ok": False,
                    "reason": f"{len(red)} case(s) failing: "
                              + ", ".join(red)}
        if vacuous:
            return {**out, "verdict": "VACUOUS", "ok": False,
                    "reason": f"{len(vacuous)} case(s) never observed RED: "
                              + ", ".join(vacuous)
                              + " — a test that has never failed has never "
                                "been tested. Run `staircase cases redcheck` "
                                "BEFORE the work, so the suite proves it can "
                                "detect the thing missing."}
        return {**out, "verdict": "GREEN", "ok": True,
                "reason": f"{green}/{len(suite)} green, all proven falsifiable"
                          f" ({', '.join(sorted(dims))})"}
...
# ------------------------------------------------------------- self-update
# ------------------------------------------------- COMPLEX TEST CASES
def _named_promises(p: Project) -> set[str]:
    tiso = _now().date().isoformat()
    return {i for pl in p.plans if pl["date"] <= tiso for i in pl["ids"]}


def run_suite(p: Project, pid: str, ledger: bool = True) -> dict:
    """Execute every case in a promise's suite and (by default) LEDGER the
    result. Ledgering every run is not bookkeeping — it is the mechanism: the
    run history is the only place falsifiability can be established, so a run
    that is not recorded cannot help a case earn its green."""
    suite = p.case_suite(pid)
    if not suite:
        raise _fail(f"no test cases for {pid!r} — author them BEFORE the work "
                    f"(`staircase cases add {pid} --name ... --dim ... "
                    "--run ... --why ...`)")
    results = []
    for name in sorted(suite):
        c = suite[name]
        proc = subprocess.run(c["run"], shell=True, capture_output=True,
                              text=True)
        results.append({
            "case": name, "dim": c["dim"], "run": c["run"],
            "status": "green" if proc.returncode == 0 else "red",
            "exit": proc.returncode,
            "tail": (proc.stderr or proc.stdout or "").strip()[-200:]})
    if ledger:
        p.append("cases.jsonl", {"type": "caserun", "id": pid,
                                 "results": results})
        p._case_events = read_jsonl(p.dir / "cases.jsonl")
...
def cmd_cases(a) -> int:
    p = load_project(a.dir)
    fn = {"add": _cases_add, "redcheck": _cases_redcheck, "run": _cases_run,
          "amend": _cases_amend, "show": _cases_show}[a.cases_cmd]
    return fn(p, a)


def _cases_add(p: Project, a) -> int:
    """Author one case. Cases are named in advance, like the promise they
    guard — that is what makes the red-check possible at all."""
    if a.id not in _named_promises(p):
        raise _fail(f"{a.id!r} is not a named promise — name it first "
                    f"(`staircase plan {a.id} --means ...`). Cases guard "
                    "promises; a case with no promise guards nothing.")
    if a.dim not in DIMENSIONS:
        raise _fail(f"--dim must be one of {', '.join(DIMENSIONS)} — "
                    + "; ".join(f"{k}: {v}" for k, v in DIMENSIONS.items()))
    if a.name in p.case_suite(a.id):
        raise _fail(f"case {a.name!r} already exists for {a.id!r}. Changing a "
                    "case after it is authored is an AMENDMENT, not an add — "
                    f"`staircase cases amend {a.id} --name {a.name} --run "
                    "... --reason ... --by ...`. (The rewrite-the-test-to-make-"
                    "it-pass move is exactly what this refusal exists to stop.)")
    if not a.why or not a.why.strip():
        raise _fail("--why is required: state what breaks in the real world if "
                    "this case passes when it should not. A case whose author "
                    "cannot say what it protects is not protecting anything.")
    p.append("cases.jsonl", {"type": "case", "id": a.id, "case": a.name,
                             "dim": a.dim, "run": a.run, "why": a.why.strip()})
    st = p.__class__(p.dir).case_status(a.id)
    print(f"staircase: case {a.name!r} [{a.dim}] added to {a.id!r} "
          f"({st['total']} case(s): {', '.join(st['dims'])})")
    if st["verdict"] == "SHALLOW":
        print(f"  SHALLOW — {st['reason']}")
    print(f"  next: `staircase cases redcheck {a.id}` BEFORE you build — every "
          "case must be seen RED to be worth anything green")
    return 0


def _cases_redcheck(p: Project, a) -> int:
    """THE RED-CHECK. Run the suite BEFORE the work and require every case to
    FAIL. This is the whole feature in one command.

    A case that is already green before you have built anything is not testing
    your promise — it is testing something that was already true. Most commonly
    it greps for a string that is always present, or it is a lazy negative case
    that "passes" because the endpoint 404s rather than because the system
    correctly refused. The red-check catches both, and it catches them at
    authoring time, when they are free to fix."""
    res = run_suite(p, a.id)
    green = [r for r in res["results"] if r["status"] == "green"]
    for r in res["results"]:
        mark = "RED " if r["status"] == "red" else "GREEN"
        print(f"  {mark} {r['case']} [{r['dim']}] (exit {r['exit']})")
    if green:
        print(f"\nRED-CHECK FAILED: {len(green)} case(s) are ALREADY GREEN "
              "before the work exists — " + ", ".join(r["case"] for r in green))
        ...
        print("Rewrite those cases to assert the SPECIFIC correct behavior, "
              "then red-check again.")
        return 1
    print(f"\nRED-CHECK PASSED: {res['total']}/{res['total']} cases RED. "
          "Every case is proven capable of failing — now their green will mean "
          "something. Go build.")
    return 0


def _cases_run(p: Project, a) -> int:
    res = run_suite(p, a.id)
    st = p.__class__(p.dir).case_status(a.id)
    for r in res["results"]:
        mark = {"green": "✓", "red": "✗"}[r["status"]]
        vac = " (VACUOUS — never seen red)" if r["case"] in st["vacuous"] else ""
        print(f"  {mark} {r['case']} [{r['dim']}] exit {r['exit']}{vac}")
        if r["status"] == "red" and r["tail"]:
            print(f"      {r['tail'].splitlines()[-1][:120]}")
    print(f"\n{st['verdict']}: {st['reason']}")
    if a.json:
        _emit_json("cases run", {**res, "status": st})
    if st["ok"]:
        print(f"  → suite is green and falsifiable: `staircase log-win {a.id} "
              "--proof ...` will be accepted")
        return 0
    # partial pass under pressure is not a failure — it is a SPLIT boundary
    if st["verdict"] == "RED" and st["green"]:
        print(f"  → {st['green']}/{st['total']} green. Under deadline pressure "
              "those green cases ARE the half that landed: "
              f"`staircase split {a.id} --into {a.id}-<green-part> "
              f"{a.id}-<red-part>` and ship the half you can prove.")
    return 1


def _cases_amend(p: Project, a) -> int:
    """The ONLY way to change an authored case — and it is deliberately loud.

    Editing the test until it goes green is the single most common way a proof
    gate dies, and it dies silently. Here it cannot: the amendment is ledgered,
    signed, dated, written into expectations.md beside the cadence history, and
    — decisively — the amended case loses its red history (falsifiability is
    keyed to the command text). A case weakened to pass is a case with no proof
    it can fail, so it reads VACUOUS and cannot bank a win until it has been
    seen red again. Which, if the code already works, means reverting the code.
    Honest either way."""
    suite = p.case_suite(a.id)
    if a.name not in suite:
        raise _fail(f"no case {a.name!r} on {a.id!r} — amend changes an "
                    "existing case; use `cases add` for a new one")
    old = suite[a.name]
    if old["run"] == a.run and (a.dim or old["dim"]) == old["dim"]:
        raise _fail("amendment is a no-op — the case is unchanged")
    p.append("cases.jsonl", {
        "type": "case", "id": a.id, "case": a.name,
        "dim": a.dim or old["dim"], "run": a.run,
        "why": a.why or old.get("why", ""), "amended": True,
        "was": old["run"], "reason": a.reason, "by": a.by})
    entry = (f"- {_now().date().isoformat()} — test case {a.name!r} on promise "
             f"{a.id!r} amended by {a.by} — {a.reason}\n"
             f"    was: `{old['run']}`\n    now: `{a.run}`")
    exp = p.dir / "expectations.md"
    text = exp.read_text() if exp.is_file() else ""
    anchor = "## Test-case amendments (a changed gate is an expectation change)"
    if anchor in text:
        ...
        text = text.rstrip("\n") + f"\n\n{anchor}\n\n{entry}\n"
    exp.write_text(text)
    print(f"staircase: case {a.name!r} on {a.id!r} AMENDED, signed by {a.by} — "
          "recorded in cases.jsonl and expectations.md (never silent)")
    print(f"  was: {old['run']}\n  now: {a.run}")
    print("\n  This case has NO red history under its new command — it is "
          "VACUOUS until observed failing again. If the implementation already "
          "makes it pass, it cannot bank a win: a check that has never caught "
          "anything has not earned the right to certify anything.")
    return 0


def _cases_show(p: Project, a) -> int:
    st = p.case_status(a.id)
    crit = p.promise_criteria().get(a.id, {})
    print(f"=== {a.id} — {crit.get('means') or 'NO definition of done'} ===")
    if not st["cases"]:
        print("  no cases authored — this promise rests on a single --accept "
              "line, or on nothing at all")
        return 1
    falsifiable = p.falsifiable_cases(a.id)
    for name in sorted(st["cases"]):
        c = st["cases"][name]
        flags = []
        if name not in falsifiable:
            flags.append("VACUOUS: never seen red")
        if c.get("amended"):
            flags.append(f"AMENDED by {c.get('by')}: {c.get('reason')}")
        print(f"  [{c['dim']:<12}] {name}"
              + (f"   ← {' · '.join(flags)}" if flags else ""))
        print(f"       run: {c['run']}")
        print(f"       why: {c.get('why', '')}")
    print(f"\n{st['verdict']}: {st['reason']}")
    if a.json:
        _emit_json("cases show", st)
