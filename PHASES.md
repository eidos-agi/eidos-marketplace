...
- **Researching `tui-forge` — the TUI design playbook forge.** Daniel feedback (2026-04-28) on `emux`'s initial TUI: "the tui sucks." Stdlib `input()` + numbered list was wrong for a human-facing surface. Reference TUIs Daniel pointed at: "cr v2" and "cockpit" — clarification pending; could be `claude-resume-duet`, `claude-boss`, `cockpit-mcp`, `ai-cockpit`, or external (lazygit, k9s, fzf, gum, textual exemplars). **Phase 2 task:** survey those references, extract patterns (keyboard nav, visual hierarchy, fuzzy filter, color, density, registry/list rendering), draft `forges/tui-forge/SKILL.md` with the playbook. Ship as a forge in Phase 6 (operational forges). Apply the playbook back to `emux`'s TUI before Phase 3d ship — the emux picker is the first customer of `tui-forge`'s output.
...
- [ ] **tui-forge** — onboard. Codifies the TUI design playbook drafted during Phase 2 (keyboard nav, visual hierarchy, fuzzy filter / list rendering, color discipline, when to use stdlib vs. textual vs. fzf-backed). Apply retroactively: bring `emux`'s picker up to the playbook bar, then re-audit. Born from the 2026-04-28 friction "the tui sucks" — first customer is emux, but every future Tool that wants a TUI surface (e.g., a future `claude-runner` UI, a future plugin browser) starts here.
